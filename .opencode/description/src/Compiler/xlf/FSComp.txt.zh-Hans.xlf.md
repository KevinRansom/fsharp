...
Pipeline role: XLIFF satellite localization resource for the FSComp compiler diagnostics string table (src/Compiler/FSComp.txt, base of FSComp.resx and the generated FSComp.SR message module) carrying the bulk of F# compiler warnings and errors; compiled into a satellite resource loaded by the resource manager for the Simplified Chinese culture.

- File format: XLIFF 1.2 XML (xliff-core-1.2-transitional) with one <file> element, source-language "en", target-language "zh-Hans", original "../FSComp.resx".
