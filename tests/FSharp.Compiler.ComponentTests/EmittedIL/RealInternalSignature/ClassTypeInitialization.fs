...
let ``for _ in Array groupBy id [||] do ...`` () = [|for _ in Array.groupBy id [||] do 0|]
    ...
    [<InlineData(false, false)>]        // Regular NoOptimize
    [<Theory>]
    let ``FlexibleMonad`` (realSig, optimize) =
        let withOptimization compilation =
            if optimize then compilation |> withOptimize
            else compilation |> withNoOptimize
...
type T =
  ...
  member i<'a,'b> : x: 'b -> y: 'a -> unit

val h1<'a,'b> : 'b * 'a -> unit

val h2<'a,'b> : x: ('b -> 'a) -> unit

val h3<'a,'b> : x: {| F1: 'b; F2: 'a |} -> unit

val h4<'a,'b> : 'b seq * (int * 'a) array -> unit

val z<'a,'z> : z1: 'z -> z2: 'z -> z3: 'a -> 'z

type IMonad<'a> =

  abstract bind<'a,'b,'b when 'a :> IMonad<'a> and 'b :> IMonad<'b>> : #IMonad<'a> -> ('a -> #IMonad<'b1>) -> IMonad<'b1>

type A<'zzz> =

  new: unit -> A<'zzz>

  static member Foo: argA2: 'a * argB2: ('a -> 'b) * argC2: ('b -> 'c) * argD: ('c -> 'd) * [<System.Runtime.InteropServices.Optional>] argZ2: 'zzz -> 'd

type C<'a> =

  new: unit -> C<'a>

  static member SM5<'b,'c> : y: 'a * z: 'b -> int"""
...
type C<'a>() =
    ...
    static member SM5<'b,'c>(y:'a,z:'b) = 2"""
        |> asLibrary
        |> withAdditionalSourceFile signature
        |> withRealInternalSignature realSig
        |> withOptimization
        |> compile
        |> shouldSucceed
