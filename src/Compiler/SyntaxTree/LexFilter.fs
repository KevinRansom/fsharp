...
type LexFilterImpl (
    ...
    let rec hwTokenFetch useBlockRule =
        ...
        let insertComingSoonTokens(keywordName, comingSoon, isHere) =
            ...
            if not enablelibraryonlyfeatures then
...
type LexFilter (indentationSyntaxStatus: IndentationAwareSyntaxStatus, enablelibraryonlyfeatures, lexer, lexbuf: Lexbuf, debug) =
    let debug = debug || forceDebug
    let inner = LexFilterImpl(indentationSyntaxStatus, enablelibraryonlyfeatures, lexer, lexbuf, debug)
